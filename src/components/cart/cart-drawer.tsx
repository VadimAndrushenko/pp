...
const ALL_DISHES = allMenuData.flatMap((menu) =>
  menu.sections.flatMap((section) =>
    section.dishes.map((dish) => ({
      key: `${menu.id}::${section.id}::${dish.name}`,
      dish,
...
const SearchResultCard = memo(function SearchResultCard({
  ...
  return (
    <div className="relative rounded-card bg-bg/60 border border-border/30 hover:border-accent/50 transition-colors duration-300">
      {prices.hasDiscount && (
      ...
      <div className="flex items-center gap-3 p-3.5">
        ...
        >
          ...
          <span className="flex items-center gap-2 mt-1 flex-wrap">
            <span className="px-1.5 py-0.5 rounded-sm text-[9px] font-display uppercase tracking-wider bg-accent/15 text-accent border border-accent/30">
              {menuGroup}
            </span>
            <span className="text-xs text-text-muted truncate">{sectionTitle}</span>
            {dish.weight && (
      ...
      {expanded && (
        <div className="animate-fade-up px-3.5 pb-3.5 space-y-2 border-t border-border/20 mt-3 pt-3 text-sm leading-relaxed">
          {dish.description && (
            <p>
...
function CartSearch({
  ...
  badge: string | null
  onBadgeChange: (v: string | null) => void
  onBack: () => void
}) {
  ...
  const results = useMemo(() => {
    ...
    return ALL_DISHES.filter(({ dish, menuTitle, sectionTitle, menuGroup }) => {
      const text = [dish.name, dish.description, dish.composition, dish.subtitle, dish.base, menuTitle, sectionTitle, menuGroup]
        .filter(Boolean)
        .join(" ")
        .toLowerCase()
  ...
  const filterKey = `${deferredQuery}\u0000${badge ?? ""}`
  ...
  return (
    <>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4">
        {results.length === 0 ? (
          <div className="animate-fade-up flex flex-col items-center justify-center gap-3 py-14 text-center">
          ...
          <div className="space-y-2.5">
            ...
            {shown.map(({ key, dish, menuGroup, sectionTitle }, i) => (
              <div
                key={key}
...
export function CartDrawer() {
  ...
  const repeatOrder = (order: OrderRecord) => {
    ...
    for (const item of order.items) {
      const entry = ALL_DISHES.find((d) => d.dish.name === item.name)
      if (!entry) continue
      for (let i = 0; i < item.quantity; i++) {
        addItem(item.name, entry.dish)
        added++
      }
    }
    if (added > 0) {
  ...
  return (
    <>
      <div aria-hidden={!isOpen} inert={!isOpen} className={`fixed inset-0 z-[80] ${isOpen ? "" : "pointer-events-none"}`}>
        <div
          onClick={handleClose}
        ...
        <aside
          id="cart-drawer"
          data-open={isOpen}
          className={`fixed top-0 right-0 bottom-0 z-[65] flex flex-col w-full max-w-[516px] max-sm:max-w-full bg-surface border-l border-border/40 shadow-[0_0_60px_rgba(0,0,0,0.7)] overscroll-contain transition-transform duration-300 ease-out ${
            isOpen ? "translate-x-0" : "translate-x-full"
          }`}
        >
          <div key={face} className="flex min-h-0 flex-1 flex-col">
            {searchOpen ? (
              <CartSearch
                query={query}
              ...
              <>
                <div className="px-5 py-4 border-b border-border/30">
                  {historyOpen ? (
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <span className="animate-scale-pop flex items-center justify-center w-10 h-10 rounded-full bg-success/15 border border-success/50 shadow-[0_0_16px_rgba(62,207,110,0.35)]">
                    ...
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex items-center gap-3 max-sm:gap-2 min-w-0 flex-1">
                        <span className="shrink-0 flex items-center justify-center w-10 h-10 max-sm:w-8 max-sm:h-8 rounded-full bg-accent/15 border border-accent/40">
                          <ShoppingBag className="w-5 h-5 max-sm:w-4 max-sm:h-4 text-accent" />
                        </span>
                        <div className="min-w-0">
                          <h2 className="font-display uppercase tracking-widest text-xl max-sm:text-lg font-bold text-text-primary truncate">
                            Корзина
                          </h2>
                          {totalCount > 0 && (
                            <p className="text-xs text-text-muted font-display uppercase tracking-wider truncate">
                      ...
                      <div className="flex items-center gap-2 max-sm:gap-1.5 shrink-0">
                        ...
                        <button
                          ...
                          className="flex items-center justify-center w-9 h-9 max-sm:w-8 max-sm:h-8 rounded-full border border-border/50 text-text-secondary transition-all duration-200 hover:text-accent hover:border-accent hover:rotate-90"
                        >
                          <X className="w-4 h-4" strokeWidth={2.2} />
                        </button>
                      </div>
                ...
                {historyOpen ? (
                  <div className="animate-fade-in flex-1 overflow-y-auto px-4 py-4 space-y-3">
                    {orders.length === 0 ? (
                      <div className="animate-fade-up flex flex-col items-center justify-center gap-3 py-14 text-center">
                      ...
                      <>
                        {orders.map((order, i) => (
                          <div
                            key={order.id}
                            className="animate-fade-up rounded-card bg-bg/60 border border-border/30 p-4"
                            style={{ animationDelay: `${Math.min(i, 8) * 45}ms` }}
                          >
                  ...
                  <div className="animate-fade-up flex-1 flex flex-col items-center justify-center gap-5 px-6 text-center">
                    <div className="animate-scale-pop flex items-center justify-center w-24 h-24 rounded-full bg-success/15 border-2 border-success shadow-[0_0_40px_rgba(62,207,110,0.45)]">
                  ...
                  <>
                    <div className="animate-fade-in flex-1 overflow-y-auto px-5 py-5 space-y-4">
                      <div>
                  ...
                  <div className="animate-fade-up flex-1 flex flex-col items-center justify-center gap-4 px-6 text-center">
                    <div className="flex items-center justify-center w-20 h-20 rounded-full border border-dashed border-accent/50 bg-accent/5">
                    ...
                    <div className="animate-fade-in flex-1 overflow-y-auto px-4 py-4 space-y-3">
                      {items.map((item, i) => {
                        ...
                        return (
                          <div
                            key={item.key}
                            className="animate-fade-up relative rounded-card bg-bg/60 border border-border/30 p-4 hover:border-accent/40 transition-colors duration-300"
                            style={{ animationDelay: `${Math.min(i, 8) * 45}ms` }}
                          >
                            <button
                            ...
                            {isExpanded && (
                              <div className="animate-fade-up mt-3 pt-3 border-t border-border/20 space-y-2 text-sm leading-relaxed">
