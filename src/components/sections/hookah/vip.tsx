...
export function HookahVIP({ className }: { className?: string }) {
  return (
    <Link
      href="/vip"
      className={`group relative overflow-hidden rounded-card border border-accent/30 block transition-all duration-300 hover:shadow-[0_0_50px_rgba(255,106,0,0.3)] hover:border-accent/80}`}
    >
      ...
      <div className="relative flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-10 py-5 sm:py-3 px-6 sm:px-8">
        <div className="flex items-center justify-between gap-4 sm:gap-10">
          <h2 className="shrink-0 text-center text-accent font-display uppercase leading-[0.9] tracking-tight transition-all duration-300 group-hover:text-white group-hover:drop-shadow-[0_0_15px_rgba(255,106,0,0.8)]">
        ...
        <div className="flex flex-1 min-w-0 items-center justify-between gap-4 sm:block">
          ...
          <span className="inline-flex items-center gap-2 px-5 py-2.5 rounded-sm border border-accent font-display uppercase tracking-wider transition-all duration-300 shrink-0 max-sm:text-sm max-sm:px-4 max-sm:py-2 group-hover:bg-accent group-hover:text-white group-hover:shadow-[0_0_25px_rgba(255,106,0,0.5)]">
            Подробнее
